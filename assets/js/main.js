...
copy.addEventListener("click", () => {
  navigator.clipboard.writeText("suhairsha07@gmail.com");
...
var messageArr = ["Tech Enthusiast", "Problem solver", "Visionary"];
...
function typewriter() {
  ...
  const message = messageArr[currentMessageIndex];
  
  if (textPosition <= message.length) {
    jobTitleElement.innerHTML = message.substring(0, textPosition);
    textPosition++;
    setTimeout(typewriter, speed);
  } else {
    textPosition = 0;
    currentMessageIndex = (currentMessageIndex + 1) % messageArr.length;
    setTimeout(typewriter, 1000); // Pause before typing the next message
  }
}
